...
def normalize_name(name):
    ...
    # Force Dynamic Star Maa Mapping
    if "maa" in name:
        # Re-format to Star Maa [Sub-channel]
...
def get_master_logo_db():
    ...
    for name, url, fmt in sources:
        ...
        except: print(f"[-] Source {name} skipped.")
    return db
    ...
    try:
        ...
        for ch in (channels or []):
            ...
            if "telugu" in p_name.lower() or "telugu" in str(ch.get('tv_genre_id', '')):
                ...
                # Star Maa Branding & Logo Sync
                if norm in logo_db:
                    final_name = logo_db[norm]['name']
                    # Ensure "Star" prefix for Maa
                ...
                if "vanitha" in final_name.lower(): final_name = "Vanitha TV"
                if "history" in final_name.lower(): final_name = "History TV18 HD Telugu"
